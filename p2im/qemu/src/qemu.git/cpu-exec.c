...
// address_range arr_addrs[MAX_ADDRESS_NUM];
// int num_addrs = 0;
...
{
	/*
	if(addrs_file && num_addrs == 0) {
		char stri[100], *pEnd;
    	uint32_t addri;
    	int32_t sizei;
    	uint8_t mode;
    	FILE *fp = fopen((char*)addrs_file, "r");
    	if(!fp) {
    	    perror("Error opening Address file\n");
    	} else {
    	    while(num_addrs < MAX_ADDRESS_NUM && fgets(stri, 100, fp) != NULL) {
    	        addri = strtol(stri, &pEnd, 0);
    	        sizei = strtol(pEnd, &pEnd, 0);
    	        mode = (uint8_t)strtol(pEnd, &pEnd, 0);
    	        arr_addrs[num_addrs].addr = addri;
    	        arr_addrs[num_addrs].size = sizei;
    	        arr_addrs[num_addrs++].mode = mode;
    	    }
    	}
    	fclose(fp);
	}
...
#define __OPEN_UNIT_TEST__
	 ...
	 if(run_from_afl && pm_stage == FUZZING && 
	  ...
	  if(pc == last_bbl_exec && size < 5) {
	    bbl_exec_times++;
	  	if(pc != main_eaddr) {
		  // when detect hang in SR_R_ID stage we try to trace back
...
//#endif
		...
		/*
		FILE *fp1 = fopen("/tmp/qemu.log", "a+");
		fprintf(fp1, "pc_s=%#x pc_e=%#x pid=%d\n", cur_bbl_s, cur_bbl_e, getpid());
		fclose(fp1);
		*/
	  } else {
		bbl_exec_times = 0;
	  ...
	  /*
	  // patch for Heat-Press
	  if (pc == main_eaddr && pm_stage == FUZZING) {
		run_main_times++;
	  }
	  if (run_main_times > 500) {
		exit(0);
	  }
	  */
#endif
                                      ...
                                      uint64_t flags)
{
 ...
 not_found:
   /* if no translated code available, then translate it now */
	...
	/*if(!first_ok && call_up_forkserver) {
    	startForkserver(env, 0);
		first_ok = true;
	}*/
#endif
