...
DEF("main-eaddr", HAS_ARG, QEMU_OPTION_main_eaddr, \
    "-main-eaddr addr \tend addr if main function in firmware\n", QEMU_ARCH_ALL)

DEF("run-from-afl", 0, QEMU_OPTION_run_from_afl, \
    "-run-from-afl fname \tpoint if qemu is called from me\n", QEMU_ARCH_ALL)

DEF("len-seeds", HAS_ARG, QEMU_OPTION_len_seeds, \
    "-len-seeds len \tlength of seeds\n", QEMU_ARCH_ALL)

DEF("len-round", HAS_ARG, QEMU_OPTION_len_round, \
    "-len-round len \tlimit of round detect\n", QEMU_ARCH_ALL)

DEF("hang-mode", HAS_ARG, QEMU_OPTION_hang_mode, \
    "-hang-mode mode \thang detect mode\n", QEMU_ARCH_ALL)

DEF("address-file", HAS_ARG, QEMU_OPTION_address_file, \
    "-address-file file \tpoint to the file which contain all valid address\n", QEMU_ARCH_ALL)
