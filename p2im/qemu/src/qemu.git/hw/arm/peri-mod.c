...
target_ulong pm_SR_read(pm_Event *e, pm_MMIORegister *reg) {
	...
	//bool factor_other = pm_rand[pm_rand_sz / 2] % NUM_CHANGE_STATUS == 1; 
#else
	if(pm_rand_sz > 0) {
	...
	if(e->satisfy_num > 0) {	
		...
		if(!e->is_result_hang && 
			...
			if(pm_rand_sz > 0 && (e->cur_other == 0 || e->num_read % 23 == 1)) {
				...
				//e->cur_other = pm_rand[pm_rand_sz == 0 ? 0 : e->num_read % pm_rand_sz] % e->other_num;
			}
			if(e->num_read % 121 == 0) {
		...
		if(pm_rand_sz > 0 && (e->cur_satisfy == 0 || e->num_read % 23 == 1)) {
			...
			//e->cur_satisfy = pm_rand[pm_rand_sz == 0 ? 0 : e->num_read % pm_rand_sz] % e->satisfy_num;
		}
		if(e->num_read % 121 == 0) {
    ...
    while(peri) {
        ...
        for (i = 0; i <= peri->max_reg_idx; i++) {
          ...
          if (type == UC) {
...
#ifdef __OPEN_MULTI_STATUS__
			  if(peri->hang_in_while_flag) {// && peri->base_addr == last_CR_SR_peri_addr) {
			...
			json_t *eval = json_object();
			char val[16], addr[16], cur_bbl_s[16], cur_bbl_e[16];
			snprintf(addr, 16, "0x%x", peri->regs[i].addr);
            json_object_set_new(jreg, "addr", json_string(addr));
            //snprintf(hex_str, 16, "0x%x", peri->regs[i].cr_val);
            //json_object_set_new(jreg, "cr_value", json_string(hex_str));
			json_object_set_new(jreg, "num_eval", 
			  			  json_integer(peri->regs[i].num_eval));
			for(int j = 0; j < peri->regs[i].num_eval; j++) {
			  snprintf(val, 16, "0x%x", peri->regs[i].evals[j].val);
			  snprintf(cur_bbl_s, 16, "0x%x", peri->regs[i].evals[j].bbl_s);
			  snprintf(cur_bbl_e, 16, "0x%x", peri->regs[i].evals[j].bbl_e);
			  eval = json_pack("{s:s, s:s, s:s}",
					"val", val,
					"cur_bbl_s", cur_bbl_s,
					"cur_bbl_e", cur_bbl_e
					 );
					//"time_rd", peri->regs[i].evals[j].time_rd,
			  json_array_append_new(evals, eval);
			}
			json_object_set_new(jreg, "evals", evals);
#endif
no_evals_dump:
			;
#endif
...
int pm_load_model(pm_Peripheral **pm_PList) {
    ...
    json_object_foreach(jperis, base_addr, jperi) {
        ...
        json_array_foreach(jregs, j, jreg) {
			...
			json_t *jeval;
	    	int jj;
			char *hex_val, *hex_pc, *hex_addr, *hex_cur_bbl_s, *hex_cur_bbl_e;
			jeval = NULL, jevals = NULL, hex_addr = NULL;
			status = json_unpack_ex(jreg, &error, 0, "{s:s, s:i, s:o}",
							"addr", &hex_addr,  
			  				"num_eval", &peri->regs[j].num_eval,
			  				"evals", &jevals
			  				);
			//if (status != 0) goto error;
			if(status == 0) {
			  if(!json_is_array(jevals)) {
        	    fprintf(stderr, "error: jevals is not an array\n");
        	    json_decref(root);
        	    return -1;
			  }
			  json_array_foreach(jevals, jj, jeval) {
				hex_val = NULL, hex_pc = NULL;
			    status = json_unpack_ex(jeval, &error, 0, 
			    	"{s?:s, s?:s, s?:s, s?:s}",
			    	"val", &hex_val,
			    	"pc", &hex_pc,
					"cur_bbl_s", &hex_cur_bbl_s,
					"cur_bbl_e", &hex_cur_bbl_e,
				);
			    //"time_rd", &peri->regs[j].evals[jj].time_rd,
			    if(status) goto error;
			    peri->regs[j].evals[jj].val = strtol(hex_val, NULL, 16);
			    peri->regs[j].evals[jj].pc = strtol(hex_pc, NULL, 16);
			    peri->regs[j].evals[jj].bbl_s = strtol(hex_cur_bbl_s, NULL, 16);
			    peri->regs[j].evals[jj].bbl_e = strtol(hex_cur_bbl_e, NULL, 16);
			  	peri->regs[j].addr = strtol(hex_addr, NULL, 16);
			  }
			}
#endif
no_evals_load:
