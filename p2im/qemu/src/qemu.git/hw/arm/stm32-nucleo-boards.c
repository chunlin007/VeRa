...
static void nucleo_l432xx_board_init_callback(MachineState *machine)
{
    cm_board_greeting(machine);

    {
        ...
        /* The board has no oscillators. */
        cm_object_property_set_int(mcu, 48000000, "hse-freq-hz"); /* 48M */
        cm_object_property_set_int(mcu, 32768, "lse-freq-hz"); /* 32khz */
...
static QEMUMachine nucleo_l432xx_machine = {
    .name = "NUCLEO-L432KC",
    .desc = "ST Nucleo Development Board for STM32 L43 series",
    .init = nucleo_l432xx_board_init_callback };
...
static void stm32_machines_init(void)
{
	...
	//chunlin
    qemu_register_machine(&nucleo_l432xx_machine);
#if 0
