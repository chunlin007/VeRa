...
def get_main_end_addr():
    nm = os.path.dirname(objdump_path) + "/arm-none-eabi-nm"
    ...
    cmd3 = objdump_path + " -d " + cfg.img + " --start-address=" + addr1 + " --stop-address=" + addr2
...
if __name__ == "__main__":
    ...
    for seed0 in os.listdir(cfg.afl_seed):
      ...
      while True: # may extract multiple rounds
        ...
        cmd_qemu = [cfg.qemu_bin, "-nographic", "-aflFile", seed,
          "-board", cfg_board[cfg.mcu], "-mcu", cfg_mcu[cfg.mcu], 
          "-image", cfg.img, "-pm-stage", "3", "-model-input", args.model_if, 
		  "-len-seeds", cfg.len_seeds, "-len-round", cfg.len_round,
		  "-hang-mode", cfg.hang_mode,
        ...
        cmd_me = [cfg.me_bin, "-c", cfg_f, "--run-num", run_num,
          "--print-to-file", "--run-from-forkserver", "1", "--afl-file", seed, 
    ...
    if 'none' in cfg.redzones:
       cmd_afl += [cfg.qemu_bin, "-nographic",
              "-board",  cfg_board[cfg.mcu] , "-mcu", cfg_mcu[cfg.mcu], 
			  "-image", cfg.img, "-main-eaddr", get_main_end_addr(), 
			  "-len-seeds", cfg.len_seeds, "-len-round", cfg.len_round,
			  "-hang-mode", cfg.hang_mode,
              "-pm-stage", "3", "-aflFile", "@@", 
          ]
