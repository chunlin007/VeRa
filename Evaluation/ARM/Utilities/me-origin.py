...
def one_sr_input_gen(sr_bits, one_cnt, one_name, prev_set_bit, set_bits):
    ...
    print_log("-----one_sr_input_gen-begin------")
    print(sr_bits)
    print(one_cnt)
    print(one_name)
    print(prev_set_bit)
    print(set_bits)
    print("-----one_sr_input_gen-end-----")
    one_sr = []
    if set_bits >= 1:
...
def input_f_writer(sr_bits, sr_num, sr_dir, content, fname, set_bits):
    print_log("-----input_f_writer-begin------")
    print(sr_bits)
    print(sr_num)
    print(sr_dir)
    print(content)
...
# Chunlin: Try to run firmware using Qemu and collect return code 
...
# Chunlin: Update model.json by comparing old and new json file
def stage1_4():
...
# Chunlin: Collect sr info that will be used to explore executation later
def stage1_5():
...
# Chunlin: Create init sr inputs by exploring execution according to
...
# Chunlin: Get callsite used status register used by disassembling firmware.elf 
...
def stage2(srr_info, sr_dir, fname_l, inv_num=1):
    ...
    if inv_num == 2: # XXX tentative impl
        ...
        callsites2 = []
        # Chunlin: callsite1->call function1 handling SR
        #          callsite2->call function1
        #          Then record return address
