    ...
    while run_num < retry_num:
        ...
        if not timeout:
            ...
            if timeout_num == 3: # and stage == 2:
                print("timeout happens, terminate automaically, return 0x23")
                signal.alarm(0)
                if stage == 1:
                    return 0x30
                elif stage == 2:
...
def stage1_5():
    ...
    print("list1 = %s" %list1)
    is_asyn_func = False
    if list1 == []: #or "IRQ" in srr_func or "irq" in srr_func:
        is_asyn_func = True
...
if __name__ == "__main__":
    ...
    cmd_base = [cfg.qemu_bin, "-verbose", "-verbose", "-d", cfg.qemu_log, "-nographic", "-board", cfg.board, "-mcu", cfg.mcu, "-image", cfg.img, "-main-eaddr", eaddr]
